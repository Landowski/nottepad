...
document.getElementById('login-form-sign-up').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
                ...
                if (user) {
                    try {
                        ...
                        if (docRef.exists) {
                            ...
                            unsubscribe();
                            reject(new Error('Не удалось создать пользовательский документ'));
                        }
                    } catch (error) {
                        unsubscribe();
                        reject(error);
            ...
            setTimeout(() => {
                unsubscribe();
                reject(new Error('Время ожидания при создании учетной записи истекло'));
            }, 10000);
        });
        ...
        if (error.code) {
            switch (error.code) {
                ...
                case 'auth/weak-password':
                    alert('Пароль слишком слаб.');
                    break;
                default:
                    alert(`Ошибка реестра: ${error.message}`);
            }
        } else {
            alert(error.message);
        }
...
document.getElementById('linkEsqueci').addEventListener('click', () => {
    ...
    if (email === '') {
        alert('Пожалуйста, заполните поле электронной почты.');
        return;
    }
    ...
    .then(() => {
        alert('Письмо для сброса пароля было отправлено на адрес ' + email);
    })
    .catch((error) => {
