            ...
            userDocRef.get().then((doc) => {
                if (doc.exists) {
                    ...
                    function deleteNote(id) {
                        ...
                        db.collection('bloco').doc(id).get()
                            .then((doc) => {
                                if (doc.exists) {
                                    const note = doc.data();
                                    noteToDeleteSpan.textContent = note.titulo || "Новый блок";
                                    deleteConfirmation.classList.remove("hidden");
                                    currentNoteId = id;
                                }
                            })
                            .catch((error) => {
                                console.error("Ошибка при поиске заметки для удаления:", error);
                            });
                    }
                    ...
                    function updateNotesOrder() {
                        ...
                        batch.commit()
                            .catch((error) => {
                                console.error("Ошибка обновления порядка заметок:", error);
                            });
                    }
                    ...
                    function selectNote(id) {
                        ...
                        db.collection('bloco').doc(id).get()
                            ...
                            .catch((error) => {
                                console.error("Ошибка выбора заметки:", error);
                            });
                    }
                    ...
                    .then(() => {
                        alert('Письмо с подтверждением отправлено на адрес ' + newEmail);
                    })
                    .catch((error) => {
                        alert('Выйдите из системы и войдите снова, чтобы иметь возможность изменить свой адрес электронной почты.');
                    });
                    });
                    ...
                    document.getElementById('botaoTrocarSenha').addEventListener('click', () => {
                        ...
                        .then(() => {
                            alert('Письмо для сброса пароля было отправлено на адрес ' + user.email);
                        })
                        .catch((error) => {
                            alert('Произошла ошибка при отправке электронного письма для сброса пароля. Проверьте правильность адреса электронной почты и повторите попытку.');
                        });
                    });
