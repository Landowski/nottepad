...
function deleteNote(id) {
    ...
    if (note) {
        noteToDeleteSpan.textContent = note.title || "Nuevo bloc";
...
document.getElementById('login-form-sign-up').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
                ...
                if (user) {
                    try {
                        ...
                        if (docRef.exists) {
                            ...
                            unsubscribe();
                            reject(new Error('No se pudo crear el documento de usuario'));
                        }
                    } catch (error) {
                        unsubscribe();
                        reject(error);
            ...
            setTimeout(() => {
                unsubscribe();
                reject(new Error('Se agotó el tiempo al crear la cuenta.'));
            }, 10000);
        });
