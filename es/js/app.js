                ...
                if (doc.exists) {
                                ...
                                querySnapshot.forEach((doc) => {
                                    ...
                                    li.innerHTML = `
                                        <div class="dragAndSpan">
                                            ...
                                            <span>${note.titulo || "Bloc"}</span>
                                        </div>
                                        <div class="note-actions">
                                            <button class="delete-note light-mode" title="Eliminar nota"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="mdi-trash-can-outline" width="19" height="19" viewBox="0 0 24 24" fill="currentColor"><path d="M9,3V4H4V6H5V19A2,2 0 0,0 7,21H17A2,2 0 0,0 19,19V6H20V4H15V3H9M7,6H17V19H7V6M9,8V17H11V8H9M13,8V17H15V8H13Z"/></svg></button>
                                        </div>`;
                    ...
                    function deleteNote(id) {
                        ...
                        db.collection('bloco').doc(id).get()
                            .then((doc) => {
                                if (doc.exists) {
                                    const note = doc.data();
                                    noteToDeleteSpan.textContent = note.titulo || "Nuevo bloc";
                                    deleteConfirmation.classList.remove("hidden");
                                    currentNoteId = id;
                                }
                            })
                            .catch((error) => {
                                console.error("Error al buscar nota para eliminar:", error);
                            });
                    }
                    ...
                    function selectNote(id) {
                        ...
                        db.collection('bloco').doc(id).get()
                            ...
                            .catch((error) => {
                                console.error("Error al seleccionar nota:", error);
                            });
                    }
                    ...
                    linkBtn.addEventListener('click', function() {
                        ...
                        const url = prompt('Insira o enlace', 'https://');
                        if (url) {
                            noteBody.focus();
                    ...
                    .then(() => {
                        alert('Correo electrónico de verificación enviado a ' + newEmail);
                    })
                    .catch((error) => {
                        alert('Cierra sesión y vuelve a iniciar sesión para poder cambiar tu correo electrónico.');
                    });
                    });
                    ...
                    document.getElementById('botaoTrocarSenha').addEventListener('click', () => {
                        ...
                        .then(() => {
                            alert('Se ha enviado un correo electrónico para restablecer la contraseña a ' + user.email);
                        })
                        .catch((error) => {
                            alert('Se produjo un error al enviar el correo electrónico para restablecer la contraseña. Comprueba que el correo electrónico sea correcto y vuelve a intentarlo.');
                        });
                    });
