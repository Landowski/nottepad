            ...
            userDocRef.get().then((doc) => {
                if (doc.exists) {
                    ...
                    function deleteNote(id) {
                        ...
                        db.collection('bloco').doc(id).get()
                            .then((doc) => {
                                if (doc.exists) {
                                    const note = doc.data();
                                    noteToDeleteSpan.textContent = note.titulo || "New note";
                                    deleteConfirmation.classList.remove("hidden");
                                    currentNoteId = id;
                                }
                            })
                            .catch((error) => {
                                console.error("Error when searching for note to delete:", error);
                            });
                    }
                    ...
                    function updateNotesOrder() {
                        ...
                        batch.commit()
                            .catch((error) => {
                                console.error("Error updating order of notes:", error);
                            });
                    }
                    ...
                    .catch((error) => {
                        alert('Log out and log in again to be able to change your email.');
                    });
                    });
