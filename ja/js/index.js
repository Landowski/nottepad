...
function deleteNote(id) {
    ...
    if (note) {
        noteToDeleteSpan.textContent = note.title || "新しいメモ";
...
document.getElementById('login-form-sign-up').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (password !== confirmPassword) {
            throw new Error('パスワードが同じではありません。');
        }
        
        if (password.length < 8) {
            throw new Error('パスワードは8文字以上でなければなりません.');
        }
        ...
        await new Promise((resolve, reject) => {
                ...
                if (user) {
                    try {
                        ...
                        if (docRef.exists) {
                            ...
                            unsubscribe();
                            reject(new Error('ユーザードキュメントの作成に失敗しました'));
                        }
                    } catch (error) {
                        unsubscribe();
                        reject(error);
            ...
            setTimeout(() => {
                unsubscribe();
                reject(new Error('アカウント作成時にタイムアウトしました'));
            }, 10000);
        });
        ...
        try {
            ...
            console.error('ユーザーの削除中にエラーが発生しました:', deleteError);
        }
...
document.getElementById('linkEsqueci').addEventListener('click', () => {
    ...
    .then(() => {
        alert('パスワードリセットメールが送信されました ' + email);
    })
    .catch((error) => {
